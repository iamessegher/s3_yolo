...
def _to_vis_3ch_from_chw(im_chw: np.ndarray) -> np.ndarray:
    if im_chw.shape[0] == 3:
        vis = im_chw.transpose(1, 2, 0)
    else:
        acc = im_chw.astype(np.float32).sum(axis=0)
        acc -= acc.min()
        if acc.max() > 0:
            acc /= acc.max()
        vis = (acc * 255).astype(np.uint8)
...
def _to_vis_3ch_from_hwc(img: np.ndarray) -> np.ndarray:
...
def plot_images(images, targets, paths=None, fname="images.jpg", names=None):
    # Plot image grid with labels (channel-agnostic)
    ...
    if np.max(images[0]) <= 1:
        images *= 255  # de-normalize

    mosaic = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))
        vis = _to_vis_3ch_from_chw(images[i])
        mosaic[y:y + h, x:x + w, :] = vis
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    if crop.ndim == 2:
        ...
        crop = _to_vis_3ch_from_hwc(crop)
    if save:
        file.parent.mkdir(parents=True, exist_ok=True)
        f = str(increment_path(file).with_suffix(".jpg"))
        Image.fromarray(crop[..., ::-1] if BGR else crop).save(f, quality=95, subsampling=0)
    return crop
...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    """
    Draw a single box on HxWxC (C>=1). If C!=3, draw on a 3-ch preview copy.
    Returns the canvas it drew on (same object if C==3).
    """
    tl = line_thickness or max(round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1, 1)
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))

    canvas = img if (img.ndim == 3 and img.shape[2] == 3 and img.dtype == np.uint8) else _to_vis_3ch_from_hwc(img)
    cv2.rectangle(canvas, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    if label:
        tf = max(tl - 1, 1)
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2_txt = (c1[0] + t_size[0], c1[1] - t_size[1] - 3)
        cv2.rectangle(canvas, c1, c2_txt, color, -1, cv2.LINE_AA)
        cv2.putText(canvas, label, (c1[0], c1[1] - 2), 0, tl / 3, (225, 255, 255), thickness=tf, lineType=cv2.LINE_AA)
    return canvas