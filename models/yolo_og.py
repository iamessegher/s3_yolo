...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.m = nn.ModuleList(QuantConv2d(x, self.no * self.na, 1,weight_quant=CommonIntWeightPerChannelQuant,weight_bit_width=4,return_quant_tensor=False)for x in ch)   # output conv
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            #### SAVE SCALES OUTPUT ####
            # output_dir = "fpga_outputs"  # Directory to save outputs
            # os.makedirs(output_dir, exist_ok=True)  # Create directory if it doesn't exist
            # output_path = os.path.join(output_dir, f"scale_{i}.npy")
            # np.save(output_path, x[i].detach().cpu().numpy())  # Save as .npy file
            # print(f"output {i} : {x[i].shape}")
            ...
            if not self.training:  # inference
                ...
                if isinstance(self, Segment):  # (boxes + masks)
                    ...
                    # xy, wh, conf = torch.nn.functional.hardtanh(x[i], min_val=0.0, max_val=1.0).split((2, 2, self.nc + 1), 4)
                    # qhardtanh01 = QuantHardTanh(min_val=0.0,max_val=1.0,bit_width=8, act_quant=CommonUintActQuant,return_quant_tensor=False )
                    # xy, wh, conf = qhardtanh01(x[i]).split((2, 2, self.nc + 1), 4)
                    xy = (xy * 2 + self.grid[i]) * self.stride[i]  # xy
                    wh = (wh * 2) ** 2 * self.anchor_grid[i]  # wh
                    y = torch.cat((xy, wh, conf), 4)
                    # print(f'Shape y {i} : {y.shape}')
                    # breakpoint()
